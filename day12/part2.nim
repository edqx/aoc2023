...
proc countNumArrangements(conditions: string, numContiguous: seq[int], memo: var Table[(string, int), int]): int =
    ...
    while j >= 0:
        ...
        if j <= 0:
            if numContiguous.len() != 1 or conditions.findLast("#") <= j + k:
                result += "".countNumArrangements(numContiguous[0..<numContiguous.len() - 1], memo)
            j -= 1
            continue
        var key = (numContiguous.join(","), j)
        if memo.hasKey(key):
            result += memo[key]
            j -= 1
            continue
        var num = conditions[0..<j - 1].countNumArrangements(numContiguous[0..<numContiguous.len() - 1], memo)
        memo[key] = num
        result += num
        j -= 1

proc loop[T](sequence: seq[T], times: int): seq[T] =
    for i in (1..times):
        result.add sequence
...
proc countNumArrangements(record: string): int =
    ...
    var memo = initTable[(string, int), int]()
    @[parts[0]].loop(5).join("?").countNumArrangements(numContiguous.loop(5), memo)
