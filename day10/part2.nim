...
var data = readFile paramStr 1
var width = data.find "\n"
sequence.addTraversedPath data
var pipes = sequence.toHashSet

var inside = 0
for i in (0..<(int)(len(data) / width)):
    var parity = false
    var lastOpen = '\0'
    for j in (i * (width + 1)..<i * (width + 1) + width):
        if j in pipes or data[j] == 'S':
            if data[j] == '|' or data[j] == 'S': parity = not parity
            if data[j] == 'L':
                parity = not parity
                lastOpen = 'L'
            if data[j] == '7' and lastOpen != 'L':
                parity = not parity
                lastOpen = '7'
            if data[j] == 'J' and lastOpen != 'F':
                parity = not parity
                lastOpen = 'J'
            if data[j] == 'F':
                parity = not parity
